...
#		pygame.mouse.set_visible(False)
		...
		self.GadgitHeight = int((self.SurfaceYLen - (2*Visual.BUTTON_HEIGHT)) / 2)
				...
				while TextLine != "":
					...
					for ThisElement in TextElements:
						if ThisElement[:5] == "Name=":
							...
							yPos = float(ThisElement[5:])
						elif ThisElement[:5] == "xLen=":
							xLen = float(ThisElement[5:])
						elif ThisElement[:5] == "yLen=":
							yLen = float(ThisElement[5:])
						elif ThisElement[:6] == "Style=":
							style = float(ThisElement[6:])
						elif ThisElement[:4] == "PID=":
							ThisPID = str(ThisElement[4:])
							ThisPidDescription = ""
							if ThisPID in ValidPIDs:
								ThisPidDescription = ValidPIDs[ThisPID]
							self.Meters[Name] = Gadgit.Gadgit(self.ThisSurface, Name, Visual.PRESS_NONE, 0, 2 * Visual.BUTTON_HEIGHT, xLen, yLen, "NEW")
							self.Meters[Name].SetStyle(style)
							self.Meters[Name].SetPos(xPos, yPos)
							#self.Meters[Name].SetLen(xLen, yLen)
		...
		if len(self.Meters) > 1:
			...
			for ThisGadget in self.Meters:
				if type(self.Meters[ThisGadget]) is not str and type(self.Meters[ThisGadget]) is not Button.Button:
					...
					Data += "|xLen=" + str(self.Meters[ThisGadget].GetXLen())
					Data += "|yLen=" + str(self.Meters[ThisGadget].GetYLen())
